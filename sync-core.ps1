...
function Publish-Pending {
    ...
    foreach ($r in $want.Keys) { if (-not $have.ContainsKey($r)) { git push -q origin "HEAD:$r" 2>$null | Out-Null } }
    foreach ($r in $have.Keys) { if (-not $want.ContainsKey($r)) { git push -q origin ":$r"      2>$null | Out-Null } }
}
...
function Report-Conflict {
    ...
    $md.Add('')
    $md.Add('The same lines were changed on both machines. Nothing was pushed and')
    $md.Add('nothing of yours was lost - your work is in local commits.')
    $md.Add('')
    $md.Add('For each file below you get three copies, side by side:')
    $md.Add('')
    $md.Add('- `*.MINE.*`   - your version, from this machine')
    $md.Add('- `*.THEIRS.*` - the other person''s version, downloaded from GitHub')
    $md.Add('- `*.BASE.*`   - the file before either of you touched it')
    $md.Add('')
...
function Invoke-Integrate {
    ...
    if ($crossed.Count -gt 0) {
        ...
        foreach ($f in $crossed) {
            ...
            git show "HEAD:$f" 2>$null | Set-Content -LiteralPath (Join-Path $crossDir "$leaf.MINE$ext")
            git show "origin/$($script:SC_Branch):$f" 2>$null | Set-Content -LiteralPath (Join-Path $crossDir "$leaf.THEIRS$ext")
            if ($mb) { git show "${mb}:$f" 2>$null | Set-Content -LiteralPath (Join-Path $crossDir "$leaf.BASE$ext") }
        }
        @("Crossed edits, merged automatically.",
          "",
          "You and your teammate changed these file(s) in the same window:",
          ...
          "  NAME.THEIRS.ext - their version, exactly as it arrived",
          "  NAME.BASE.ext   - the common version both of you started from",
          "",
          "Open the live file(s) and give the combined result a final human look.") |
            ForEach-Object { $_ } | Set-Content -LiteralPath (Join-Path $crossDir 'CROSSED.md')
    }
